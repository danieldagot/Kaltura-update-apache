   ...
   stage('Upload Cookbook to Chef Server, Converge Nodes') {
            steps {
                ...
                withCredentials([zip(credentialsId: 'chef-server-cred', variable: 'CHEFREPO')]) {
                    ...
                    sh "knife ssl fetch -c $CHEFREPO/chef-repo/.chef/config.rb "
                    //update cookbook
                    sh "knife cookbook upload apache --force -o $CHEFREPO/chef-repo/cookbooks -c $WORKSPACE/chef-repo/.chef/config.rb"
                    withCredentials([sshUserPrivateKey(credentialsId: 'ubuntu', keyFileVariable: 'AGENT_SSHKEY', passphraseVariable: '', usernameVariable: '')]) {
                        ...
                        script{
                                ...
                                {
                                    echo "ok 0 "
                                    sh " knife ec2 server create -c $CHEFREPO/chef-repo/.chef/config.rb --groups=default   --aws-secret-access-key=$AWS_SECRET_ACCESS_KEY --aws-access-key-id=$AWS_ACCESS_KEY_ID --region=us-east-1  --image=ami-013f17f36f8b1fefb --flavor=t2..micro  -u ubuntu  --ssh-key=jenkins-aws-key -i=$AGENT_SSHKEY --aws-tag Name='webserver node'  -r 'role[apache]' "
