        ...
        withCredentials(bindings: [zip(credentialsId: 'chef-server-cred', variable: 'CHEFREPO')]) {
          ...
          withCredentials(bindings: [sshUserPrivateKey(credentialsId: 'ubuntu', keyFileVariable: 'AGENT_SSHKEY', passphraseVariable: '', usernameVariable: '')]) {
            withCredentials(bindings: [aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'awsCredentialId', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
              script {
                env.countInstenses= sh (returnStdout: true, script:"knife count -c $CHEFREPO/chef-repo/.chef/config.rb name:'$params.AWS_DEFAULT_REGION-webserver' ").trim()
                ...
                if("$env.countInstenses" == '0')
                {
                  ...
                  sh "knife node run_list add '$params.AWS_DEFAULT_REGION-webserver' 'recipe[apache::default]' -c $CHEFREPO/chef-repo/.chef/config.rb "
                }
                ...
                sh "knife ssh 'name:'$params.AWS_DEFAULT_REGION-webserver'' -x ubuntu -i $AGENT_SSHKEY 'sudo chef-client' -c $CHEFREPO/chef-repo/.chef/config.rb"    
              }
