                ...
                withCredentials([zip(credentialsId: 'chef-server-cred', variable: 'CHEFREPO')]) {
                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'ubuntu', keyFileVariable: 'AGENT_SSHKEY', passphraseVariable: '', usernameVariable: '')]) {
                        ...
                        script{
                                ...
                                {
                                    echo "ok 0 "
                                    sh " knife ec2 server create  --groups=default   --aws-secret-access-key=$AWS_SECRET_ACCESS_KEY --aws-access-key-id=$awsCredentialId --region=us-east-1  --image=ami-013f17f36f8b1fefb --flavor=t2.small  -x ubuntu  --ssh-key=013f17f36f8b1fefb -i= --aws-tag Name='webserver node' -j '{ 'username' : 'daniel dagot' }' -r 'role[webserver]' "
                                }
