                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'ubuntu', keyFileVariable: 'AGENT_SSHKEY', passphraseVariable: '', usernameVariable: '')]) {
                        ...
                        script{
                                  ...
                                  if("$env.countInstenses" == '0')
                                {
                                    ...
                                    sh " knife ec2 server create -c $CHEFREPO/chef-repo/.chef/config.rb --groups=default   --aws-secret-access-key=$AWS_SECRET_ACCESS_KEY --aws-access-key-id=$AWS_ACCESS_KEY_ID --region=us-east-1  --image=ami-013f17f36f8b1fefb --flavor=t2.micro  --ssh-user ubuntu  --aws-tag Name='webserver node'  -r 'role[apache]' -y"
