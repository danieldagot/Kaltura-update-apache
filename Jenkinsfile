            ...
            withCredentials(bindings: [aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'awsCredentialId', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
              script {
                ...
                if("$env.countInstenses" == '0')
                {
                  ...
                  sh 'knife node run_list add a1 "recipe[apache::default]"'
                  // sh "knife ssh 'name:a1' -x ubuntu -i $AGENT_SSHKEY 'sudo chef-client -z -o apache ' -c $CHEFREPO/chef-repo/.chef/config.rb"    
